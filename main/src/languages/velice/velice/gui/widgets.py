...
class Widget:
    ...
    def _set_bold(self, v):
        if self.tk is None: return
        try:
            old = self.tk.cget("font")
            if isinstance(old, str) and "bold" not in old.lower():
                self.tk.configure(font=(old, "bold"))
            elif isinstance(old, (list, tuple)):
                parts = list(old)
                if len(parts) >= 2 and "bold" not in [str(p).lower() for p in parts]:
                    parts.append("bold")
                    self.tk.configure(font=tuple(parts))
        except Exception:
            pass
